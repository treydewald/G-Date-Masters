...
// count how many times the user swipes, up or down
var thumbsUpCount=0;
...
const SwipeScreen = (props) => {
  ...
  const hydratedValidRecommendation = (otherUser) => {
    ...
    const otherUserProfileIsPublic =
      otherUser.settings && otherUser.settings.show_me != null
        ...
        : true;
    const otherUserRole = otherUser.role || "null"
    ...
    if (
      ...
      otherUserRole.toLowerCase() == "bot"
    ) {
      console.log(`User Match ID: ${id} - Name: ${firstName}`)
  ...
  const onSwipe = (type, swipeItem) => {
    ...
    console.log("Likes: " + thumbsUpCount);
    console.log("Dislikes: " + thumbsDownCount);
    
    // if threshhold is met, get the top 2 rated matches
    if (thumbsUpCount === 2 || thumbsDownCount === 2){
      var allMatches=[];
    const db = firebase.firestore();
    var matchesRef = db.collection("rating");
    matchesRef.where("user", "==", user.id).orderBy("rating", "desc").limit(2)
    .get()
    .then(function(querySnapshot) {
        querySnapshot.forEach(documentSnapshot => {
          console.log('Match ID: ', documentSnapshot.get("match"), documentSnapshot.get("rating"));
          allMatches.push(documentSnapshot.get("match"));
          console.log("matches: "+allMatches);
          
          // add these matches to the user collect 
          updateDoc('users', user.id, {matches: allMatches});

          // find the info for the first date
          const dbUsers = firebase.firestore().collection('users').doc(allMatches[0]);
          dbUsers.get().then(function(doc) {
            if (doc.exists) {
              var dateName=doc.data().firstName;
              var dateUrl=doc.data().profilePictureURL;
              console.log(dateUrl);

              // send the user to the first predate
