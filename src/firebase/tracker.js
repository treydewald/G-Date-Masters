...
export default class SwipeTracker {
  ...
  addSwipe = (fromUser, toUser, type, callback) => {
    ...
    // User has swiped on a bot account, insert to  db
    firebaseSwipe.addSwipe(fromUser.id, toUser.id, type, (response) => {
      callback(response);
    });
    if (type != 'dislike') {
      // Insert the same type of swipe but for the bot to the user
      firebaseSwipe.addSwipe(toUser.id, fromUser.id, type, (response) => {
        callback(response);
      });
    }
  };
